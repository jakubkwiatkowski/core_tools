...
class Flatten2D(Layer):
    """Variation Flatten layer. Flattens the input to 2D tensor. Does not affect the batch size and last dimeetion.
    All inner dimention our squeezed into 1.
    ...
    def call(self, inputs):
        ...
        if tf.executing_eagerly():
            ...
            if rank == 1:
                ...
                non_batch_dims = input_shape[1:-1]
                ...
                if non_batch_dims.is_fully_defined():
                    ...
                    flattened_shape = tf.constant([-1, last_dim, input_shape[-1]])
                elif batch_dim is not None:
                    flattened_shape = tf.constant([int(batch_dim), -1, input_shape[-1]])
                else:
                    flattened_shape = [tf.shape(inputs)[0], -1, input_shape[-1]]
                return tf.reshape(inputs, flattened_shape)
